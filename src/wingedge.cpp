...
void WingedEdge::make(Primitive &p) {
	...
	for (size_t i=0; i < p.getVBO().mVertices.size(); i+=3){
		shared_ptr<WE_Vertex> sv (new WE_Vertex);
		vs.push_back(sv);
		sv->x = p.getVBO().mVertices[i];
		sv->y = p.getVBO().mVertices[i+1];
		sv->z = p.getVBO().mVertices[i+2];
	}
	...
	for (size_t i = 0; i < p.getVBO().mIndices.size() / 3; i+=3) {

		shared_ptr<WE_Face>  f (new WE_Face());
		bool bf = false;
		uint32_t idcs[3];
		idcs[0] = p.getVBO().mIndices[i];
		idcs[1] = p.getVBO().mIndices[i+1];
		idcs[2] = p.getVBO().mIndices[i+2]; 
