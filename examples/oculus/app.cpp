...
OculusApp::OculusApp() {
  ...
  std::function<void()> init = std::bind(&OculusApp::Init, this);
  std::function<void(double_t)> draw = std::bind(&OculusApp::Draw, this, std::placeholders::_1);
  const GLWindow &windowZero = window_manager_.CreateWindow("Oculus Application", 1280, 800, init, draw, true, "HDMI-0", -1,-1 );
}
...
void OculusApp::ThreadMainLoop(double_t dt) {    
  oculus_.Update(dt);
}
 ...
 */
 ...
 void OculusApp::Draw(double_t dt){
...
void OculusApp::MainLoop(double_t dt) {
  window_manager_.MainLoop();
}


void OculusApp::ProcessEvent( const GLWindow &window, CloseWindowEvent e) {
 ...
 */
 ...
 void OculusApp::ProcessEvent( const GLWindow &window, MouseEvent e){
 }
 ...
 */
 ...
 void OculusApp::ProcessEvent( const GLWindow &window, ResizeEvent e){
...
void OculusApp::ProcessEvent( const GLWindow &window, KeyboardEvent e){
